...
while true; do
    ...
    if [ $mem_porcentaje -ge 90 ]; then 
        ...
        echo -e "[!] La RAM esta al $mem_porcentaje de uso $(date)\n" >> /var/log/recursos.log
    else 
    ...
    if [ $uso_real -ge 90 ]; then 
        ...
        echo -e "[!] El procesador esta al $uso_real de uso $(date)\n" >> /var/log/recursos.log
    else 
        uso_real="$uso_real%"
    fi
    ...
    echo -e "RAM \tCPU\n$mem_porcentaje\t$uso_real"
    echo -e "\n\e[34mProceso con mas memoria\n\e[0m$(ps aux --sort=-%mem | head -2; for user in $(w -h | awk '{print $1}' | sort -u); do ps aux --sort=-%mem | awk -v u=$user '$1 == u {print; exit}'; done)"
    echo -e "\n\e[34mProceso con mas uso del procesador\n\e[0m$(ps aux --sort=-%cpu | head -2; for user in $(w -h | awk '{print $1}' | sort -u); do ps aux --sort=-%cpu | awk -v u=$user '$1 == u {print; exit}'; done)"
    echo -e "\n\e[34mEspacio libre en discos\e[0m\n$(df -h | head -1)\n$(df -h | grep /dev/sd)"
    echo -e "\n\e[34mDiscos\e[0m\n$(iostat | tail -6 | head -4)"
    ...
    for i in $(ip -br link show | awk '$2 == "UP" {print}' | wc -l); do 
        ...
        subida=$((subida / 1024))
        echo -e "\e[34m$interfaz:\e[0m ↓ $bajada KB | ↑ $subida KB"
    done
done